#!/usr/bin/env python3
"""Utility to convert cookies/headers to curl command"""
...
def cookies_to_string(cookies_dict: dict) -> str:
    """Convert cookies dictionary to cookie header string."""
    return "; ".join([f"{key}={value}" for key, value in cookies_dict.items()])


def build_curl_command(url: str, cookies: dict, headers: dict, data: str = None) -> str:
...
def main():
    ...
    # Read input files
    print("Reading files...")
    cookies = read_json_file(Path("cookies/aa_cookies.json"))
    headers = read_json_file(Path("cookies/aa_cookies_headers.json"))
    referer = read_text_file(Path("cookies/aa_cookies_referer.txt"))
